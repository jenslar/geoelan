...
pub(crate) fn sensor2plot(
    ...
    match sensor_type {
        ...
        _ => {
            // 3D sensors
            ((y_axis_x, y_axis_y), y_axis_z) = sensor_data.iter()
                .cloned()
    ...
    let x_axis: Vec<f64> = match x_axis.map(|s| s.as_str()) {
        ...
        Some("c" | "count") => {
            ...
            (0..y_axis_x.len()).into_iter().map(|i| (i + 1) as f64).collect::<Vec<_>>()
        }
        other => {
    ...
    let y_axis_label = Title::new(&format!("{} ({})", sensor_type.quantifier(), sensor_type.units()));
    ...
    return Ok((
        ...
        y_axis_label,
        vec![
            Scatter::new(x_axis.to_owned(), y_axis_x).name("x"), // TODO add units to x-axis
