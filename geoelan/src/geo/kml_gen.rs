...
use kml::{
    types::{AltitudeMode, Coord, Element, Geometry, LineString, LinearRing, Placemark, Point},
    Kml, KmlDocument,
};
...
pub fn kml_timestamp(
    datetime_start: &PrimitiveDateTime,
    datetime_end: Option<&PrimitiveDateTime>,
) -> Element {
...
pub fn kml_point(
    ...
    let mut children: Vec<Element> = point
        .datetime
...
pub fn kml_linestring(
    ...
    let mut children: Vec<Element> = match (
        points.first().and_then(|p| p.datetime),
        points.last().and_then(|p| p.datetime),
    ) {
        (Some(t1), Some(t2)) => {
    ...
    if let Some(h) = height {
        linestring.coords.iter_mut().for_each(|c| c.z = Some(*h));
...
pub fn placemarks_from_geoshape(
    ...
    match geoshape {
        ...
        | GeoShape::PointSingle { height } => points
            .iter()
            .enumerate()
            .map(|(i, point)| {
                let style = point
