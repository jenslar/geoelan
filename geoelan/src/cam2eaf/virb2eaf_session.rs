...
pub fn run(args: &clap::ArgMatches, virb_session: &mut VirbSession) -> std::io::Result<()> {
    ...
    let geotier = args.contains_id("geotier");
    ...
    if let Ok(gps) = virb_session.gps() {
        if gps.is_empty() {
            ...
            let mut cluster = EafPointCluster::from_virb(&gps, None, &t0, &end, Some(time_offset as i64))
    ...
    let session_start_ms = virb_session.start
        .map(|n| n.whole_milliseconds() as i64);
