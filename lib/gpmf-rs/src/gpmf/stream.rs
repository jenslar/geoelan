...
use crate::{ContentType, GpmfError, gopro::Dvid};
...
impl Stream {
    ...
    /// - `None` is returned from if `StreamType::Nested`.
    ...
    /// Find first stream with specified `ContentType`.
    pub fn filter(&self, content_type: &ContentType) -> Vec<Self> {
        match &self.streams {
    ...
    /// Find all streams with specified `StreamType`.
    pub fn filter_all(&self, content_type: &ContentType, recursive: bool) -> Option<&Self> {
        match &self.streams {
    ...
    /// passing `FourCC::DVNM` to `find()` instead.
